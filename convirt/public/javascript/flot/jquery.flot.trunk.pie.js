    ...
    function Plot(target_, data_, options_) {
        ...
        function fillInSeriesOptions() {
            ...
            for (i = 0; i < series.length; ++i) {
                ...
                // pie chart have no instance options
                //s.pies = $.extend(true, {}, options.pies, s.pies);
        ...
        function setupGrid() {
            ...
            setSpacing();
            if (!options.pies.show) insertLabels();
            insertLegend();
        }
        ...
        function shuffleColors(colorSet) {
            var idx = Math.floor(Math.random() * colorSet.length);
            var idxArray = [];
            idxArray.push(idx);
            while(idxArray.length < colorSet.length) {
                idx = Math.floor(Math.random() * colorSet.length);
                while(arrayIndexOf(idx, idxArray) != -1) {
                    idx = Math.floor(Math.random() * colorSet.length);
                }
                idxArray.push(idx);
            }

            var colorSetShuffled = [];
            for(var i = 0 ; i < idxArray.length; i++) {
                colorSetShuffled.push(colorSet[idxArray[i]]);
            }
            return colorSetShuffled;
        }
        ...
        function drawPies(series) {

            ctx.save();
            ctx.clearRect(0, 0, canvasWidth, canvasHeight);
            ctx.translate(plotOffset.left / 2, plotOffset.top / 2);

            target.find(".pieLabels").remove();
            var html = ['<div class="pieLabels">'];
            ...
            for(var i = 0; i < series.length; i++) {
                ...
                for(var j = 0; j < series[i].data.length; j++) {
                    series[i].value += new Number(series[i].data[j][1]);
                }
                sumSeries += series[i].value;
                series[i].datapoints.points = null; // FIXME for findNearbyItem
            }

            var radius;
            if (options.pies.radius) {
                radius = options.pies.radius;
            } else {
                radius = plotHeight * 0.95 / 2;
                if (options.legend.show) radius *= 0.9;
            }
            var labelRadius = options.pies.labelRadius || radius * 0.6;

            // setup label formatter
            var lf;
            if (typeof options.pies.labelFormatter == 'function') {
                lf = options.pies.labelFormatter;
            } else if (options.pies.labelFormatter == 'label') {
                lf = function(label, value, pct, textVal, pie, s, o) {
                    return label;
                }
            } else if (options.pies.labelFormatter == 'value') {
                lf = function(label, value, pct, textVal, pie, s, o) {
                    return value;
                }
            } else if (options.pies.labelFormatter == 'percent') {
                lf = function(label, value, pct, textVal, pie, s, o) {
                    return textVal;
                }
            } else if (options.pies.labelFormatter == 'full') {
                lf = function(label, value, pct, textVal, pie, s, o) {
                    return textVal + '%<br><span style="font-size: 75%">' + label + '<br>(' + value + ')</span>';
                };
            } else {
                lf = function(label, value, pct, textVal, pie, s, o) {
                    var br, subLabel;
                    if (o.pies.bias && pct < o.pies.bias[2] &&
                        Math.abs(Math.sin(pie.sliceMiddle)) < 0.5)
                        br = '';
                    else
                        br = '<br>';
                    if ((0.97 < pie.sumSeries && pie.sumSeries < 1.03) || 
                        (  97 < pie.sumSeries && pie.sumSeries < 103))
                        subLabel = label;    // value is equal with percent
                    else
                        subLabel = label + br + '(' + value + ')';    // show real value
                    if (o.pies.bias && pct < o.pies.bias[2]) {
                        return '<span style="font-size: 75%">' + textVal + '%' + br + subLabel + '</span>';
                    } else {
                        return textVal + '%<br><span style="font-size: 75%">' + subLabel + '</span>';
                    }
                };
            } 
            ...
            var startAngle = options.pies.startAngle || 0;
            var endAngle = 0;

            // adjust position of small percent
            var bias = 0, biasStep = 0, biasThreshold = 0;
            if (options.pies.bias) {
                bias          = options.pies.bias[0];
                biasStep      = options.pies.bias[1];
                biasThreshold = options.pies.bias[2];
            }

            var bgColor = parseColor(options.grid.backgroundColor || '#ffffff');

            for(i = 0; i < series.length; i++) {
                var color = parseColor(series[i].color);
                var pct = series[i].value / sumSeries;
                startAngle = endAngle;
                endAngle += (series[i].value == 0) ? 0.0001*(2*Math.PI) : pct*(2*Math.PI);
                ...
                ctx.closePath();
                ctx.fillStyle = color.scale(null, null, null, 0.8).toString();
                ...
                if (color.toString() != bgColor.toString()) {
                    ...
                    var left = labelX;
                    var top  = labelY;
                    if (options.pies.bias && pct < biasThreshold) {
                        left += plotWidth * Math.cos(sliceMiddle) * (0.1 - pct) * bias;
                        top  += plotWidth * Math.sin(sliceMiddle) * (0.1 - pct) * bias;
                        bias *= biasStep;
                        if (clamp(-2.0, bias, 2.0) != bias) bias = options.pies.bias[0];
                    }

                    var textVal = Math.round(pct * 100);
                    if (i == series.length - 1 && textVal + totalPct != 100) {
                        textVal = 100 - totalPct;
                    }
                    totalPct = totalPct + textVal;

                    // format label
                    var pie = { radius: radius, labelRadius: labelRadius, sumSeries: sumSeries,
                        anchorX: anchorX, anchorY: anchorY, labelX: left, labelY: top,
                        startAngle: startAngle, endAngle: endAngle, sliceMiddle: sliceMiddle };
                    var label = lf(series[i].label, series[i].value, pct, textVal, pie, series[i], options);
                    html.push('<div class="pieLabel" style="position: absolute; left: '+ left + 'px; top:'+ top + 'px;' + options.pies.labelStyle + '">' + label + '</div>');
                }
            }        
